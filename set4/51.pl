...
$eventfilter->{"query"} = 
	...
	"AND TargetInstance.Name<>'D' " . 
	"AND TargetInstance.Name<>'E' ";
...
$scriptconsumer->{"name"} = "usb_script_consumer";
$scriptconsumer->{"scriptingengine"} = "PerlScript";
$scriptconsumer->{"scripttext"} = 
	'open FILE, ">>C:\\\\usb.txt"; ' . 
	'print FILE "USB %TargetInstance.Name% inserted! \n";';
